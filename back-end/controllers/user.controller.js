...
userCtl.authenticateLegacy = async (req, res) => {
  const { email, password } = req.body;

  User.getUserByUsername(email, (err, pet) => {
    if (err) throw err;
    if (!pet) {
      return res.json({ success: false, msg: 'Email not found' });
    }

    User.comparePassword(password, pet.password, (err, isMatch) => {
      if (err) throw err;
      if (isMatch) {
...
userCtl.me = async (req, res) => {
  try {
    const user = await User.findById(req.user.id); // Cambiado de req.query.id a req.user.id
    if (!user) {
  ...
  if (user) {
    ...
    if (user.isActivated) {
      ...
      if (req.query.idSecond != '0') {
        const data = user.newPetProfile.find(
          (x) => x._id == req.query.idSecond
        );
        res.status(200).send({ success: true, payload: data });
      } else {
        const {
...
userCtl.editPhotoProfile = async (req, res) => {
  ...
  const result = await cloudinary.uploader.upload(
    req.file != undefined ? req.file.path : req.body.image,
    { folder: 'mascotas_cr' }
  );
